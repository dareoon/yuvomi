...
**No self-made duplicates (GHSA-6pmj-w42g-g6qv).** Whoever writes an address to a linked contact
without being an admin - the member on their own profile (`PATCH /api/v1/auth/me/profile`) or own
contact (`PUT /api/v1/contacts/:id`), or a group manager creating a shared-expense guest
(`POST /api/v1/split-expenses/groups/:id/guests`, or `.../members` with a free `contact_id`) - is
refused with 409 and `reason: "email_in_use"` when a newly introduced address already belongs to
another account. The check is `emailsTakenByOtherAccounts()` in the same file: the same
`accountIdsByEmail()` rule as SSO linking and password reset (primary and additional addresses,
surrounding whitespace, ASCII letter case), linked accounts included, split-expense guests excluded
(they never link through their address, and one person can be a guest in two groups). Only newly
introduced addresses count, so a form saved unchanged stays savable even where a duplicate predates
the check. An admin may assign one address to several accounts on purpose (a family mailbox); the SSO
sign-in then refuses the address as ambiguous instead of guessing.
